  ...
  class UPS < Carrier
    ...
    def build_location_node(xml, name, location, options = {})
      ...
      xml.public_send(name) do
        ...
        if (name == 'Shipper') && (origin_account = options[:origin_account] || @options[:origin_account])
          xml.ShipperNumber(origin_account)
        elsif (name == 'ShipTo') && (destination_account = options[:destination_account] || @options[:destination_account])
