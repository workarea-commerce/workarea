module ActiveShipping
  ...
  class USPS < Carrier
    ...
    def package_valid_for_max_dimensions(package, dimensions)
      ((not ([:length, :width, :height].map { |dim| dimensions[dim].nil? || dimensions[dim].to_f >= package.inches(dim).to_f }.include?(false))) &&
              (dimensions[:weight].nil? || dimensions[:weight] >= package.pounds) &&
              (dimensions[:length_plus_girth].nil? ||
                  (dimensions[:length_plus_girth].to_f >=
                  package.inches(:length) + package.inches(:girth))) &&
              (dimensions[:length_plus_width_plus_height].nil? ||
