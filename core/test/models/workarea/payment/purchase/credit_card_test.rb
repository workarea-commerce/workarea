    ...
    module Purchase
      class CreditCardTest < Workarea::TestCase
        def test_complete_does_nothing_if_gateway_storage_fails
          ...
          StoreCreditCard.any_instance.expects(:save!).returns(false)

          Workarea.config.gateways.credit_card.expects(:purchase).never
        ...
        def test_complete_sets_the_transaction_attributes_on_a_failure_response
          ...
          refute(transaction.success?, "expected transaction.response? to be false")
          assert_instance_of(
            ActiveMerchant::Billing::Response,
            transaction.response
          )
